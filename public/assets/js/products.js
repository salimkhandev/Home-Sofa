...
function openProductModal(productId) {
    ...
    document.getElementById('modalMainImage').src = product.mainImage;
    ...
    document.getElementById('modalAvailability').textContent = product.available ? 'In Stock' : 'Out of Stock';
    document.getElementById('modalAvailability').style.color = product.available ? 'var(--success)' : 'var(--danger)';
    
    // Update WhatsApp link
    const whatsappLink = document.getElementById('modalWhatsapp');
    whatsappLink.href = `https://wa.me/971500000000?text=I'm interested in ${encodeURIComponent(product.name)} - AED ${product.price.toLocaleString()}`;
    ...
    thumbnailsContainer.innerHTML = product.galleryImages.map((img, index) => `
